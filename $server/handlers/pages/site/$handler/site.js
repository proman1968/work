export default {
    ...
    <div class="chrome" flex>
        <div class="tabs-row" id="tabs-row" ~class="{stacked: tabsStacked}">
            ...
            <div ~if="isTop" class="user-slot" id="user-slot">
                <oda-button
                    ~if="isLoggedIn"
                    class="explorer-btn"
        ...
        <div class="sheet" flex>
            <iframe ~for="frames" ~show="current_href === $for.item.href" :src="$for.item.href" @load="on_frame_load($event)"></iframe>
        </div>
    </div>
    `,
    default_view: 'site-main',
    current_href: '',
    current_item: null,
    frames: [],
    _frameSeq: 0,
    ...
    open_explorer() {
        if (!this.$item) return;
        window.open(this.$item.url + '/~/handlers//explorer/index.html', '_blank');
    },
    async open_profile() {
    ...
    frame_url(item) {
        ...
        return new URL(item.url + '/~/handlers//site-navigation/index.html').href;
    },
    ...
    async emit_location() {
        ...
        if (this.current_item && this.isSelf(this.current_item)) {
            loc = buildSiteLoc(myShort, '', { view: this.default_view });
        } else {
    ...
    on_frame_load(e) {
        const el = e?.target;
        const frame = this.frames.find(f => f.href === el?.src);
        if (!frame) return;
        frame.el = el;
    },
    _onMessage(e) {
    ...
    updateTabsStacked() {
        ...
        const stacked = this.tabsStacked
            ? available < 480
            : available < 400;
