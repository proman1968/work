...
Тип `$task` — JSON-носитель длинного диалога/PDCA (`ai.task`): `type: 'task'` + `items` (+ `todo`, `mode` `plan`|`build`, `system`, `model`, `goal`) и **session-`prompt`** на самом типе файла.
