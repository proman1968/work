...
| [`ui/panel.js`](ui/panel.js) | Action-bar + `work-prompt-bar`. `:model` / `:effort` one-way от `data` + `@model-changed.stop` / `@effort-changed.stop`; `::tts-mode`; `:pending` с шелла. Mic в баре. `actionButton`: строка `stop` → `APPROVE`; `stop: true` → нет кнопки; «Продолжить» только `halt` stop/crash. Form: JSON `$pdp.result` до `pending`. Send из инпута — `userRole`. |
