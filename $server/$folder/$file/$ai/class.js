 ...
 *
 * prompt — автомат:
 * 1. двухтактный ход: такт 1 «думай» (functions не передаются, весь ответ = thinking)
 *    → такт 2 «маршрут» — одно слово research | plan | task | action | report | cancel
 *    → обработчик `_<слово>`; не слово — ответ пользователю блоком text (wait);
 * 2. research — второй выбор словом: web | work | form | question | ask_user;
 * 3. plan | report → md-блок type plan|report + кнопка «Принять» (wait);
 *    «Принять» плана → сразу _task (без второго маршрута словом);
 *    task → to-do список → блок task (steps) + самовызов первого пункта;
 *    action | web | work → FC-ход: один вызов функции (functions из схемы класса,
 *    /SERVICES/* и harness) либо ответ текстом;
 * 4. вызовы, меняющие файлы, → body.pendingAction + кнопка «Выполнить» (wait);
 *    read-only исполняются сразу → блоки tool / tool_result → самовызов-продолжение;
 * 5. реальный вход с confirm/answers (post JSON микрочата): pendingAction —
 *    выполнить/отменить; «Принять» плана → сразу _task; «Принять» отчёта →
 *    task.closed + выход на родительский ribbon (если есть);
 * 6. слово task без принятого plan и без active task → принудительно _plan;
 *    при active task — инъекция текущего пункта на острие _thinking;
 *    все steps done → coerce report; complete_step двигает прогресс;
 * 7. продолжения — this.async(() => this.prompt({role:'ASSISTENT', prompt, _turn})),
 *    лимит MAX_AUTO_TURNS → кнопка «Продолжить» (полный двухтакт на каждом ходе).
 *
...
/** Слова-маршруты второго такта → обработчики `_<слово>`. */
const ROUTES = ['research', 'plan', 'task', 'action', 'report', 'cancel'];
const ACTION_PROMPT = ['Исходя из твоих размышлений выше, сделай ровно одно действие:',
    '\n[instruction]',
    'Ответь ровно ОДНИМ словом, без знаков препинания и пояснений:',
    'research — если необходимо что-то исследовать;',
    'plan — если необходимо согласовать план (новый общий план с нуля);',
    'task — только если пользователь принял plan, либо нужна декомпозиция текущего пункта активной задачи;',
    'action - если необходимо выполнить одно действие;',
    'report — если все пункты задачи закрыты или пора предоставить отчёт.',
    'cancel — если пользователь отменил действие.',
].join('\n');
/** Слова второго выбора research → обработчики `_<слово>`. */
const RESEARCH_ROUTES = ['web', 'work', 'form', 'question', 'ask_user'];
const RESEARCH_PROMPT = ['Подумай хорошенько, и выбери один из вариантов:',
    '\n[instruction]',
    'Ответь ровно ОДНИМ словом, без знаков препинания и пояснений:',
    'web — если необходимо найти что-то в интернете;',
    'work — если необходимо найти что-то в рабочей области;',
    'form — если необходимо заполнить форму;',
    'question - если необходимо задать вопросы с вариантами ответов;',
    'ask_user — если необходимо задать один уточняющий вопрос;',
].join('\n');
...
export default {
    ...
    /**
     * Автомат task.ai.
     * Реальный вход (role USER|BOSS|ADMIN): text → блок prompt + двухтактный ход;
     * confirm/answers (post JSON микрочата) → _confirm.
     ...
     * @param {string} [post] JSON микрочата { prompt, model, role, confirm, answers }
     */
    async prompt(params = {}, post) {
        if (typeof post === 'string' && post.trim().startsWith('{')) {
            try { Object.assign(params, JSON.parse(post)); }
            catch { params.prompt ??= post; }
        }
        ...
        try{
            ...
            if(!isService){
                ...
                if(params.confirm !== undefined || params.answers){
                    prompt = await this._confirm(params, user);
                    if(prompt){
                        this.async(()=>{
                            this.prompt({
                                role: 'ASSISTENT',
    ...
    /**
     * Реальный confirm/answers микрочата: pendingAction — выполнить/отменить вызовы;
     * «Принять» плана / отчёта; ответы опроса/формы → значения в поля.
     * @returns {Promise<string|undefined>} промпт продолжения для ASSISTENT-самовызова
     */
    async _confirm(params, user){
        ...
        // «Принять» плана → сразу _task (не через _thinking / слово-маршрут)
        if(open.type === 'plan' || open.title === 'План'){
            const messages = await this.context();
            return this._task(messages, user);
        }
    ...
    /**
     * Двухтактный ход: такт 1 «думай» → блок thinking; такт 2 «маршрут» —
     * слово из ROUTES → обработчик `_<слово>`, иначе ответ пользователю → блок text.
     * Ворота: task без принятого plan и без active task → _plan;
     * все steps done → coerce report. При active task — инъекция пункта на острие.
     * @returns {Promise<string|undefined>} промпт продолжения для ASSISTENT-самовызова
     */
    async _thinking(prompt, userSession){
        const inject = await this._step_injection();
        prompt = String(prompt || '') + inject + THINKING_PROMPT;
        let messages = await this.context();
        if(messages.last?.role === 'user')
            messages.last.content = prompt;
        else
            messages.push({ role: 'user', content: prompt });
        let {content, usage} = await this.streamChat({messages}, userSession)
        if(this._stopped)
            return;
        ...
        ({content, usage} = await this.streamChat({messages}, userSession));
        if(this._stopped)
            return;

        let route = content.trim().replace(/[.!]+$/, '').toLowerCase();
        // Все пункты закрыты — только report (или cancel), не новый plan/task/action
        const task = await this._active_task();
        const allDone = !!(task?.steps?.length && task.steps.every(s => s.status === 'done'));
        if(allDone && route !== 'report' && route !== 'cancel')
            route = 'report';
        if(ROUTES.includes(route)){
            messages.pop();
            // Ворота task: без принятого plan и без active task → plan
            if(route === 'task'){
                const ready = await this._plan_ready_for_task();
                const active = await this._active_task();
                if(!ready && !active)
                    return this._plan(messages, userSession);
            }
            return this['_' + route](messages, userSession);
        }
        // Не слово-маршрут — ответ пользователю: фиксируем текстом (wait)
        await this._push_block(userSession,{
            type: 'text',
            content,
            usage
    ...
    /** Маршрут research: второй выбор словом (web|work|form|question|ask_user). */
    async _research(messages, userSession){
        messages.push({
            role: 'user',
        ...
        if(this._stopped)
            return;
        const route = content.trim().replace(/[.!]+$/, '').toLowerCase();
        if(RESEARCH_ROUTES.includes(route)){
            messages.pop();
            return this['_' + route](messages, userSession);
        }
        await this._push_block(userSession,{
    ...
    async _plan(messages, userSession){
        messages.push({
            role: 'user',
            content: PLAN_PROMPT
        });
        ...
        await this._push_block(userSession, {
            ...
            button: {
                label: 'Принять',
                icon: 'vscode:file-type-light-todo'
            }
        })
    },
    async _report(messages, userSession){
        messages.push({
            role: 'user',
    ...
    /** Маршрут task: to-do из согласованного плана → блок task + первый пункт. */
    async _task(messages, userSession){
        messages.push({
            role: 'user',
            content: TASK_PROMPT
        });
        ...
        if(!steps.length){
            ...
            await this._push_block(userSession, { type: 'text', content, usage });
            return;
        }
        steps[0].status = 'in_progress';
        await this._push_block(userSession, {
            type: 'task',
            label: steps[0].description.slice(0, 120),
            content: steps.map(s => s.step + '. ' + s.description).join('\n'),
            state: 'active',
            steps,
            ribbon: [],
            usage,
        });
        return 'Делай пункт 1 плана: «' + steps[0].description + '». Ровно одно действие;'
            + ' закрыв пункт результатом — complete_step({step: 1, summary: "что сделано"}).';
    },
    /** Маршрут action: FC-ход — один вызов функции или ответ текстом. */
    async _action(messages, userSession){
        return this._fc_move(messages, userSession, DO_PROMPT);
    },
    /** Маршрут cancel: закрыть активную задачу. */
    async _cancel(messages, userSession){
        const task = await this._active_task();
        if(task){
        ...
        await this._push_block(userSession, {
            type: 'text',
            content: task ? 'Задача отменена: ' + (task.label || '') : 'Действие отменено.',
        });
    },
    // --- Ветки research ---
    async _web(messages, userSession){
        return this._fc_move(messages, userSession, WEB_PROMPT, ['search', 'fetch_url']);
    },
    async _work(messages, userSession){
        return this._fc_move(messages, userSession, WORK_PROMPT, WORK_READ_METHODS);
    },
    async _question(messages, userSession){
        return this._fc_move(messages, userSession, QUESTION_PROMPT, ['ask_user'], 'questions');
    },
    async _form(messages, userSession){
        return this._fc_move(messages, userSession, FORM_PROMPT, ['ask_user'], 'form');
    },
    async _ask_user(messages, userSession){
        messages.push({
            role: 'user',
    ...
    /**
     * FC-ход: functions в запрос (при allow — отфильтрованные),
     * один вызов функции → _handle_call; нет вызова → ответ текстом (wait).
     * @returns {Promise<string|undefined>} промпт продолжения
     */
    async _fc_move(messages, userSession, prompt, allow, askType){
        messages.push({ role: 'user', content: prompt });
        let functions = await this.functions();
        if(allow)
            functions = functions.filter(f => allow.includes(f.name));
        const {content, usage, calls} = await this.streamChat({messages, functions}, userSession);
        if(this._stopped)
            return;
        const call = calls[0];
        if(!call){
        ...
        return this._handle_call(call, userSession, askType);
    },
    /**
    ...
    /**
     * В активной ленте есть answered plan, после которого ещё нет task —
     * слово task разрешено (to-do из согласованного плана).
     */
    async _plan_ready_for_task(){
        const ribbon = await this.get_active_ribbon();
        let planIdx = -1;
        for(let i = 0; i < ribbon.length; i++){
            if(ribbon[i].type === 'plan' && ribbon[i].answered)
                planIdx = i;
        }
        if(planIdx < 0)
            return false;
        return !ribbon.slice(planIdx + 1).some(b => b.type === 'task');
    },
    /**
     * Инъекция текущего пункта / прогресса на острие _thinking (не в ленту).
