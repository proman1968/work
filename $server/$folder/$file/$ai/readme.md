...
3. `prompt` — **инстанс-метод файла** (наследуется из `class.js` типизатора через merge-цепочку, `this` = файл `task.ai`). Реальный text → блок `prompt` в активный ribbon → `_walk(PIPE)`. Служебный вход — сразу `_walk(PIPE)` без блока.
4. **Конечный автомат `PIPE`** (дерево состояний, один движок `_walk`). Узел = состояние: `prompt` (генерация/инъекция), `inject` (подсказка меню родителя), `next` (дети), `button` (wait), `fc` (function-calling), `askType`. Корень — `thinking`.
   ...
   - **Лист**: `fc`-узел (`web`/`file`/`action`) → `_handle_call` + continue на `thinking`; без `fc` (`step`) → отрендеренный `prompt` как continue-строка (ASSISTENT → `thinking`).
   - **Ворот `allDone`** в `thinking`: все steps done → форс `report`. При active task — инъекция текущего пункта (`_step_injection`) на острие.
5. **Подтверждение изменений.** Любой метод, меняющий файлы (`save_file`/`edit`/`write_file` + `WRITE_METHODS`), — **только через кнопку**: `body.pendingAction = {calls}` + action «Выполнить», без trust-автопропуска. Read-only вызовы исполняются сразу (`_execute_call`) → `tool`+`tool_result` → continue. `ask_user({questions})` → блок `questions`/`form` (wait).
...
- `class.js` — **весь ИИ-харнесс**: дерево `PIPE` (конечный автомат), движок `_walk`, методы `prompt` / `stop` / `change_model`, tools + ACL, контекст пары
...
- ✅ Конечный автомат `PIPE`: узлы-состояния (`thinking`/`plan`/`task`/`step`/`research`/`search`/`web`/`file`/`ask`/`form`/`questions`/`text`/`action`/`report`), `inject`/`next`/`button`/`fc`; один движок `_walk` (вместо `_<route>` хендлеров и `_confirm`)
...
- ✅ FC-узлы: `web`/`file`/`action` — `streamChat({functions})` → один вызов или text (wait); `form`/`questions` — `ask_user`
...
- ✅ Подтверждение изменений: `WRITE_METHODS` → `pendingAction` + «Выполнить»; read-only — сразу; `ask_user` → `questions`/`form`
