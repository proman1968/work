        ...
        try {
            ...
            if (next.length) {
                ...
                if (next.length === 1)
                    ...
                    menu = menu.join('\n');
                    console.log(params.block.type, menu);
                ...
                if (choice) {
                    ...
                    await this._push_block(params);
                    const text = next_pipe.prompt || PIPE[params.container.type]?.prompt;
                    if(!params.block.container && !params.block.content && text){
                        let messages = await this.context({prompt: text, session});
