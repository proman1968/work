...
- [`class.js`](/$server/$folder/$file/$task/class.js/~/handlers/pages/form/) — харнесс: `prompt`, `context`, `_pipe_stream`, `_streamChat`, `_fc_exec`, `_push_block`, `_active_*`, `stop`, `change_model`, `_save`, `_continue`; `CONTINUE`; цикл: `next_options`, `useBlock`, `can_close`, `attachFiles`, `stageOpen`, `containerMode`, `timeNow`
