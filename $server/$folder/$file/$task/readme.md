...
Инвариант: нет `content` — блок живой (в него спускаемся); есть `content` — завершён. Листья получают `content` из стрима или FC; внутренний контейнер — когда `report` (`close`) написал отчёт (`recalc` кладёт текст в `container.content` и вырезает лист из ленты). `continue` / имя шага — отклонить (площадку не закрывать, лист всё равно вырезать, `cut` сбрасывает окно `limit`). `complete` — шаг корня (кнопка), не «итог задачи» в system. Тот же признак у `step` (`todo.recalc` смотрит `st.content`). `state` — подпись сути в шапке (не фаза), сначала текст, потом число: `web` — `Сайты 2/2` (успешные / все `site` в ленте), `explore` — `Интернет 1, Форма 2`, `todo` — `2/5 Шаг`, мыслей нет (справа `time`). APPROVE — тоже `state`.

1. **`on_save`** пишет `body.system` (SYSTEM + режимы `plan`/`do` + профиль / рабочая группа; если `path` совпал — один блок «личная зона») и вызывает `file.prompt`. Если в `items` уже есть `prompt` — `role: 'AI'`, без второго user-блока; иначе `prompt` из `body.title`. Чат без текста кладёт `prompt` из имён файлов (не «есть вложения»).
2. **Роли входа `prompt()`:**
...
3. **`PIPE`** — константа в [`class.js`](/$server/$folder/$file/$task/class.js/~/handlers/pages/form/) (не отдельный `pipe.js`). Узел = метаописание типа блока. Корень файла — контейнер `task` (`body.type ??= 'task'`). Метод `prompt()` не ветвится по `type`, кроме входа `prompt`.
   - `plan` / `do` → `{ next: [...] }` — маршруты контейнера по `container.mode || 'plan'`; свой `next` блока важнее `content` (после `prompt` всегда `thinking`); нет своего — меню контейнера; подряд — `container.last`;
   ...
   - `limit` — есть ли тип в меню: в окне `afterLastPrompt` / `cut` уже `>= limit` — типа нет. Квоты всех таких типов кончились — только `report`. `continue` → `cut`. Нет поля — без потолка; новый `prompt` снимает `cut`;
   ...
   - `close` — лист закрывает родителя (`report`): отчёт → `container.content`, затем splice. `continue` / имя шага — отклонить, splice, `cut` = длина `items`, снять `last`. В `_container_context` узлы `close` не идут в messages. `report` снова в меню, когда есть факты и ни кэш, ни хвост ленты не `report`;
   ...
   - `icon` — спокойная иконка в JSON; живая — только UI (`typeIcon` / `streamTarget`); `fc` — зона вызова (`/` = WORK или путь сервиса); `schema` / `allow` на узле (файловые атомы) или `SCHEMA` сервиса (`web` / `site`); `role`, `build` — где заданы;
...
5. **Один шаг `prompt()` после входа:**
   ...
   - иначе `next_options`: свой `next` важнее `content`; закрытый контейнер без `done.next` — меню родителя; лист без `next` — родитель; из меню вычитается `container.last`; тип с `limit` — нет, если в окне уже `>= limit`; все квоты кончились — только `report`; `close` — только если `can_close` (есть факты); пустое меню — `chat.done`;
   ...
   - `parse?`, `recalc`, `close_up` (узел и предки; если лист уже вырезан — с контейнера). У `report` (`close`): сводка → `container.content` или reject + `cut`; лист из ленты. Auto-loop после меню: `!block.stop` → `this.async` следующего `prompt({ role: 'AI' })` и `return` без `chat.done`. После `run` — `_continue`, если не `_stopped`. `chat.done` — только когда автомат остановился. `_stopped` сбрасывается на USER/APPROVE. Выбор не из меню — узел с `fallback`.
6. **Атомы `web` / `site`** (как `todo` / `step`):
   - `web` — поиск: `system` от `plan`/`do`, `next` = `site`, `report`, `limit: 1`. Запрос — `webQuery`: свой `label`, иначе `prompt`/`title`, иначе первая строка `thinking`. Пропуск «есть вложения» / `task` / ярлыка «Интернет». `web.sites` — `{ url, title }` из выдачи, без url соседей того же родителя. Обход — до `site.limit` успешных (`siteOk`: `content` и не `error`); капча / пусто — `error`, квоту не занимают, берём следующий url. `webPushNext` из `web.run` / `site.run`. `state` — `Сайты` успешные/всего в ленте. `report` — только если есть `siteOk`. В сводку — **Источники** (`formatSites`);
   - `site` — `limit: 1` по успешным. `run`: `fetch_url`, капча/пусто → `error` без обзора; иначе стрим (`site.prompt`). `label` — title выдачи.
...
13. **Complete / report:** `complete` — шаг корня, `stop: 'Принять'`. `accept` без `prompt` → approved, `task.content = block.content`; иначе `rejected`. `report` — подробный md-отчёт этапа → `container.content`, лист вырезается. `recalc` дописывает галерею из детей (`formatGallery`: `![]` / видео, без url уже в тексте; alt — `decodePct`, слаг Canva не пишем) и список **Источники**. Схема — общий `MERMAID` в `report.prompt` и `web.close_prompt`. `continue` / имя шага — отклонить, лист вырезать, `cut` как новый промпт (без фейкового `prompt` в контекст). Новый user-prompt снимает `content` корня и открывает цикл.
14. **Контекст:** `_container_context(container)` — один слой: `system` из `PIPE` (`node[mode].system` / `node.system`, корень — `body.system`) + `[todo]` + `items` как листья (без спуска в `b.items`; узлы `close` / `report` в messages не входят). Открытый контейнер без `content` — в ленту слот «Текущий этап далее (label).» (этап = блоки после маркера, не ярлык). Закрытый — свой `content`. `context()` — путь от корня до активного, склейка `system` слоёв + **сейчас** (`hereNow` каждый вызов: дата/время в `session.here.tz`) + точка, если браузер прислал `here` (на `session`, не на диск) + фраза режима; место в запросе важнее GPS. Ленты через один `push` (два assistant подряд — «продолжай»), хвостовой instruction к последнему user. `prompt()` пишет `session.here` из query `here` (JSON: `lat`, `lon`, `tz`).
...
- ✅ подряд в меню — `container.last`; квоты `limit` кончились — только `report`
...
- ✅ `explore` / `work` / `web` / `includes` закрывает `report` (сводка на контейнере, лист вырезается; иначе `continue` = отклонить + `cut`)
- ✅ `web` сам вызывает `search` (не ждёт function_call модели)
- ✅ `site.content` — разбор страницы в md (факты, прайсы, ссылки, `![ ]` / видео из хвостов дампа), не дамп `fetch_url`
- ✅ `limit` в PIPE — окно `afterLastPrompt` / `cut`; исчерпание → только `report`; `web.sites` — `{ url, title }`; в сводке **Источники**
