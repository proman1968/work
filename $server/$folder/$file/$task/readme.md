...
1. **`on_save`** пишет `body.system` (SYSTEM + **выбор шага** + режимы `plan`/`do` + профиль / рабочая группа; если `path` совпал — один блок «личная зона») и вызывает `file.prompt` с `here` из чата / панели. Если в `items` уже есть блоки — `role: 'AI'`, без user-блока и без `title` в задачу. Иначе `prompt` из `body.title`. Чат без текста — блока `prompt` нет; имена файлов только во вложениях. Старый таск держит старый `body.system`, пока снова не сработает `on_save`.
2. **Роли входа `prompt()`:**
...
14. **Контекст:** `_container_context(container)` — один слой: `system` из `PIPE` + `[todo]` + `items` как листья (узлы `close` не входят). У открытого `site` — `block.page` (уже с `[site N: url]`). Открытый контейнер без `content` — слот «Текущий этап далее (label).». `context()` — путь от корня + **сейчас** (`hereNow`) + режим. `keepHere` пишет `session.here` и `body.here` (`tz`, lat/lon). `fillHerePlace` — Nominatim reverse → `here.place`. `hereNow`: «Пользователь сейчас в точке … (город). Это его город, пока он не назвал другое.»
