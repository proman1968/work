...
5. **Закрытие goal:** `need=facts` — evidence сбора/реплики (`explore`/`web`/`logs`/`answer`/`report`) → `done`. `need=side` — после work проверка агентом [`check`](/$server/$folder/$class/ai/agents/check.js/~/handlers/pages/form/) (`exist`/`meta` → `goalDone`); терминальный `answer` при open → pursue. При `status=done` «Продолжить» (`role:AI`) не показывается и на сервере no-op.
