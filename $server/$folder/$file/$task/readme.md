...
8. **Режимы:** меню корня — `PIPE.task.plan` / `PIPE.task.do` (в обоих есть `complete` — итог всего таска). `explore` не меняет `mode`. Закрытие обзора — `done.prompt` (есть закрытый `web`, все дети с `content`). `check` — только выход из `work` / `do`. Один `todo` на контейнер.
9. **Form / text:** два и больше вопроса — `form`. Сначала домен, потом кардинальность, потом type. Перечислимый — radio/select/checkbox + «Другое», не text с «например» и не `ul/li`. `parse` срезает script, `oda-icon`, `button` и submit. Раскладка: `fieldset`+`legend`. `text` — дрифт меню. `stop: true`.
...
11. **Complete:** отчёт — результат из ленты (факты, таблицы), не пересказ процесса. `stop: 'Принять'`. `accept` без `prompt` → approved, `container.content = block.content`; закрытый контейнер — `next_options` пуст, `chat.done`. Новый user-prompt снимает `content` и открывает цикл. Отказ или комментарий — `rejected`.
