...
6. **Атомы `web` / `site`** (как `todo` / `step`):
   - `web` — поиск: `system` от `plan`/`do`, `next` = `site`, `report`, `limit: 1`. `web.sites` — `{ url, title }` из выдачи, без url соседей того же родителя. Обход — до `site.limit` успешных в окне `afterLastPrompt` (`content` и не `error`); ошибка не считается, берём следующий. `webPushNext` из `web.run` / `site.run`. `state` — `Сайты` успешные/всего в ленте. Закрытие — `report` с `close_prompt`; в сводку — markdown-список **Источники** (`formatSites`);
   - `site` — `limit: 3`. `run`: `fetch_url` на своём `url`, затем стрим (`site.prompt`): с страницы — факты, прайсы, таблицы, ссылки, `![ ]` из `[images]`, `[подпись](url)` из `[video]` (clip режет только текст). `content` — разбор в md (или короткая ошибка), не сырой текст. `label` — title выдачи. Следующий `site` — после закрытия текущего, пока успешных меньше `limit`.
...
13. **Complete / report:** `complete` — шаг корня, `stop: 'Принять'`. `accept` без `prompt` → approved, `task.content = block.content`; иначе `rejected`. `report` — подробный md-отчёт этапа → `container.content`, лист вырезается. `recalc` дописывает галерею из детей (`formatGallery`: `![]` / видео, без url уже в тексте) и список **Источники**. Схема — общий `MERMAID` в `report.prompt` и `web.close_prompt`. `continue` / имя шага — отклонить, лист вырезать, `cut` как новый промпт (без фейкового `prompt` в контекст). Новый user-prompt снимает `content` корня и открывает цикл.
