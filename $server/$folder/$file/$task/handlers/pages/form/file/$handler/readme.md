...
- Топ-лента (`microchat-ribbon` + `$item`): scroll follow только при `stickBottom`. Отключение follow — только по событиям ввода (`wheel` вверх / `touchmove` / drag скроллбара): `scroll` генерирует и сама докрутка, при плотном стриме события пользователя тонули в её потоке. Возврат в хвост (`nearBottom`) включает follow обратно.
...
- Form-слот: колонка (`--vertical`); fieldset `max-width: 400px`; default `microchat-form` рисует `data.html`. Поле ввода у «Другое» скрыто, пока пункт не выбран. `data.values` пишутся только на `@change` (на `@input` — лишь показ «Другое»): мутация data на каждый символ перерисовывала форму и теряла ввод; `restore()` не трогает контрол в фокусе.
...
| [`ui/views.js`](ui/views.js) | Ribbon + views. Form-слот / html-iframe. Scroll: `stickBottom` (стоп — wheel/touch/drag). Sticky: `layoutTick`. |
