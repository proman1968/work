...
- Action-bar: `~if="!pending && actionButton?.label"`. Роль на кнопке: строковый `stop` — `APPROVE` + крестик (нет при `streamTarget`); «Продолжить» (`role: 'AI'`) только если хвост не `prompt` и не `stop: true` и `goal.status !== 'done'`. `sendAction` шлёт `role` кнопки; `userRole` только у send из инпута.
...
- Html-слот: `type === 'html'` → `microchat-html` `iframe[srcdoc]` + sandbox (`allow-scripts`); страница из `content` (`unwrapFence`). Лента — высота по контенту (`HEIGHT_PING`, не `100vh`; `50vh` только до замера); док — `fill` iframe `inset: 0` в лист, без ping.
...
- **Action:** роль с кнопки. Строковый `stop` — `APPROVE` + `accept` + крестик (`null` при `streamTarget`); form: `prompt` = JSON `$pdp.result`. Иначе «Продолжить» (`role: 'AI'`) при открытом корне, не `pending`/`streaming`/`stop: true`, хвост не `prompt`, `goal.status !== 'done'`. Модель в `prompt` не передаётся — она в `body.model`, смена только `change_model`.
- **Form:** слот при разметке в `content` (fence); `result` — снимок контролов; оболочка пробрасывает `view.result` → `$pdp.result`.
- **Html:** `type === 'html'` → `content` (`unwrapFence`) в `microchat-html` `iframe srcdoc` (`allow-scripts`); лента — ping по контенту; док — `fill` `inset: 0`; без APPROVE.
- **Pinned:** авто-open у `focusedBlock` и предков на спуске; без шеврона. Не на пути — стрелка свободная.
