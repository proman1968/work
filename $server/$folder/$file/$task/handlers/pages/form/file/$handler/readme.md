...
- `pinned` — авто-open у `focusedBlock` и предков на пути к нему; шеврона нет (закрыть нельзя). Сосед / закрытая площадка не на пути — сворачивается свободно.
