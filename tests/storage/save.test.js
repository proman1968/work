...
function getDifferenceGolden() {
    ...
    return { new_data, old_data, diff: $storage.getDifference(new_data, old_data) };
}
...
function separateInheritGolden() {
    ...
    const [self_data, inherit_data] = $storage.separateInheritData(new_data);
    return { new_data, self_data, inherit_data };
}

describe('$storage.getDifference', () => {
    it('returns only keys that differ from baseline', () => {
        ...
        const diff = $storage.getDifference(incoming, baseline);
    ...
    it('returns incoming when baseline is empty', () => {
        ...
        assert.deepEqual($storage.getDifference(incoming, {}), incoming);
    });
    ...
    it('does not include unchanged fields with id in arrays', () => {
        ...
        const diff = $storage.getDifference(new_data, old_data);
...
describe('$storage.separateInheritData', () => {
    ...
    it('puts to_inherit fields into inherit part', () => {
        const [self, inherit] = $storage.separateInheritData({
            label: 'Mine',
    ...
    it('returns empty inherit for plain delta', () => {
        const [self, inherit] = $storage.separateInheritData({ label: 'Only self' });
