...
import { $folder, $storage, $file } from '../sources/server/index.js';
...
describe('$folder.sortItems', () => {
    it('puts storage items (with meta folder) before plain folders and files', () => {
        ...
        const handler = Object.assign(new $storage({ id: 'chat' }, parent), { type: '$handler' });
        const user = Object.assign(new $storage({ id: 'ivan' }, parent), { type: '$user' });
