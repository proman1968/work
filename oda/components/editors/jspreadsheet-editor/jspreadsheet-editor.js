...
<script>
    ...
    let grid = window.gridJspreadsheet = jspreadsheet(document.getElementById('spreadsheet'), ${this.data});
    if (grid?.[0]?.parent) {
        ...
        setTimeout(() => document.dispatchEvent(new CustomEvent('is-ok', { detail: grid })), 100)
            ...
            `
            let blob = new Blob([srcdoc], { type: 'text/html' });
