...
import { MERGE } from './babel-merge.js';
import { installPackageSpawn } from './package-install.js';
...
export class WorkServer extends CORE.$storage {
    parent = null;
    path = '';
    // labelPath = '';
    dir = '.';
    ...
    get $folder(){
        return CORE.$folder.build('$folder', this.meta_folder);
    }
    // get $storage(){
    //     return $folder.build('$storage', this.$folder);
    // }
    get $users(){
        return this._get_item('users', CORE.$user);
    }
    get id(){
        return 'WORK';
    }
    ...
    static async mergeFiles(files = [], reset = false){
        let dirs = files.map(f=>f.real_dir).unique();
        let key = dirs.join(';');

        return  this.merges[key] ??=  new AsyncPromise(async _ =>{
            let body = '';
            switch(files[0].ext){
                case 'js':{
                    let dir
                    while(dir = dirs.shift()){
                        let next = await fsp.readFile(dir, {encoding: 'utf-8'});
                        if(body)
    ...
    static async awaitRequestResult(url, headers) {
        return new Promise((resolve, reject) => {
            const checkStatus = async () => {
                try {
                    const taskInfo = await WorkServer.request(url, headers);
    ...
    // static async downloadRquestResult(url) {
    //     return new Promise((resolve, reject) => {
    //     const client = url.startsWith('https') ? $server.https : $server.http;
    //     const chunks = [];
...
WorkServer.steps = Object.create(null);
Object.assign(WorkServer.prototype, authMethods);
globalThis.$server = WorkServer;