...
function ddgAnomaly(status, html) {
    if (status === 202)
        return true;
    return /anomaly|captcha|challenge/i.test(html) && !/\bresult__a\b|\bresult-link\b/.test(html);
}

function parseDdgHtml(html = '') {
    const results = [];
    const seen = new Set();
    const aRe = /<a\b[^>]*(?:\bresult__a\b|\bresult-link\b)[^>]*>/gi;
    let m;
...
async function fetchDdgPage(url) {
    const response = await fetch(url, {
        headers: { 'User-Agent': UA, Accept: 'text/html', Referer: 'https://html.duckduckgo.com/' },
...
async function searchHtml(query) {
    ...
    let page = await fetchDdgPage('https://html.duckduckgo.com/html/?q=' + q);
    if (ddgAnomaly(page.status, page.html) || !parseDdgHtml(page.html).length)
        page = await fetchDdgPage('https://lite.duckduckgo.com/lite/?q=' + q);
    if (ddgAnomaly(page.status, page.html))
        throw new Error('DuckDuckGo HTML challenge ' + page.status);
    return parseDdgHtml(page.html);
}
    ...
    async search(params = {}) {
        ...
        if (!out.abstract && !out.results.length) {
            try {
                ...
                htmlError = e.message || 'DuckDuckGo HTML';
                console.warn('[DuckDuckGo HTML]:', htmlError);
            }
        }
        if (!out.abstract && !out.results.length)
            return { error: htmlError || 'Ничего не найдено', query };
        return out;
    },
