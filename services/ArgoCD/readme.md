...
Унифицирует подключение к Argo CD: параметры соединения и шаблон Application описываются один раз в METADATA.FIELDS, token хранится в `#secret`, а заявки клиентов обрабатываются серверными методами. PaaS и иные производные сервисы наследуют поведение через `~`.

## Как это работает

### Подключение и token

- Параметры подключения и шаблон Application — поля `METADATA.FIELDS` (`url`, `project`, `repoURL`, `chart`, `targetRevision`, `destinationServer`, `destinationNamespace`, `syncPrune`, `syncSelfHeal`, `insecure`); значения — корневые props.
...
- Представление `settings` — редактор FIELDS + отдельный блок token (password + «задан / не задан»).
...
- [`$service/$folder/$class/$service/class.js`]($service/$folder/$class/$service/class.js) — базовый класс `$service`: METADATA.FIELDS, статусы, все серверные методы.
...
- [`PaaS/$service/...`] — производный сервис PaaS (`/SERVICES/ArgoCD/PaaS`): наследует FIELDS и методы, добавляет `baseDomain` и тарифы; значения подключения живут в самом сервисе (отдельный `prod` не используется).

## В каком это состоянии

- ✅ METADATA.FIELDS подключения и шаблон Application (значения на корне).
