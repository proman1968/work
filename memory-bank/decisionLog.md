## Развитие ИИ-архитектуры: on_save, план, SYSTEM_PROMPT — 2026-07-17

**Контекст.** При анализе ИИ-инфраструктуры обнаружено: (1) `on_save` trigger работает со старым форматом `body.chat`, который заменён на `body.ribbon` — триггер никогда не запускался. (2) Клиентский `pendingPlan` — мёртвый код, сервер никогда не устанавливает это поле. (3) Поиск `plan_created` блока в `prompt` не находил ничего — `parseResponseToRibbon` создаёт `type:'block'`, не `plan_created`. (4) SYSTEM_PROMPT содержал неточности про подтверждение плана.

**Решение.**
1. `on_save` — все обращения `body.chat` заменены на `body.ribbon`, извлечение промпта через `ribbon.find(m => m.role === 'user')`
2. `pendingPlan` — удалён из `onAction()` и `onCancelAction()` клиента
3. `planBlock` — исправлен тип поиска (`'block' && b.steps`), добавлено обновление шагов в активной задаче и проверка завершения (все `done` → `state = 'completed'`)
4. SYSTEM_PROMPT — PDCA-цикл переписан: подтверждение через `<action>`, обновление статусов, автоматическое завершение

**Правило.** Единый формат ленты — `body.ribbon`. Поле `body.chat` не существует. План завершается автоматически при `steps.every(s => s.status === 'done')`.

## Стабилизация: подтверждение опасных действий + chat/ribbon — 2026-07-17

**Контекст.** Подтверждение опасных действий было объявлено в коде (`DANGEROUS_METHODS`, `TRUST_AUTOCONFIRM`), но не реализовано — константы висели мёртвым грузом. Клиент отправлял `{confirm:true}`, но сервер его не обрабатывал. Одновременно в клиенте оставались обращения к старому `body.chat` при наличии нового `body.ribbon`, что вызывало silent-ошибки (`onFormAnswer` → `this.chat` → undefined).

**Решение.**
1. `prompt.execute()` — при обнаружении опасных методов и `trustLevel < 3`: сохранение в `body.pendingAction`, WS `chat.action`, прерывание цикла
2. При следующем вызове с `{confirm:true}` — выполнение отложенных вызовов через `executeToolCall()`, при `{confirm:false}` — tool_result "отменено"
3. Рефакторинг: вынос `executeToolCall()`, `buildFunctionsList()`, `pushToolResult()`, `sendToolResultWs()` — единые функции вместо дублирования
4. Клиент: `onFormAnswer` ищет в `taskBody.ribbon`, `_loadTaskBody` работает только с `ribbon`, убраны мёртвые `this.chat`/`this.chatGroups`

**Правило.** Опасные действия (write_file, set_property, save_file, delete, create) требуют подтверждения при `trustLevel < 3`. При `trustLevel ≥ 3` — автоподтверждение. Единый формат ленты — `body.ribbon`, `body.chat` не существует.

## functionCalling — свойство модели вместо хардкода провайдеров — 2026-07-16

**Контекст.** streamChat проверял ai.protocol !== "gigachat" для пропуска functions. Это нарушает принцип: универсальный код не должен знать о конкретных провайдерах.

**Решение.** Добавлено свойство functionCalling (Boolean) в METADATA/STATIC прототипа $ai. По умолчанию false. Модели с поддержкой (z.ai) указывают functionCalling: true. streamChat проверяет ai.functionCalling === true.

**Правило.** Универсальный код не должен знать о конкретных провайдерах. Все особенности — свойства модели.

## web_search + IP-геолокация — 2026-07-16

**Контекст.** ИИ не имел доступа к интернету. Для поиска погоды и другой информации нужен web_search.

**Решение.** Метод web_search в $ai/methods/ (DuckDuckGo + Wikipedia). IP-геолокация через ip-api.com — добавляет город в body.context и к локальным поисковым запросам.

**Правило.** web_search — безопасный метод (только чтение), добавляется в functions всегда.

## trustLevel — подготовка шкалы доверия — 2026-07-16

**Контекст.** Нужна шкала 0-5: 0=запрещено, 5=полная свобода. Пока только поле в METADATA.

**На будущее.** Реализовать проверку trustLevel в цикле prompt для автоподтверждения опасных методов.

## Подтверждение опасных действий через <action> — 2026-07-16

**Контекст.** Режим Plan/Act (кнопка run) был удалён, но полная свобода ИИ без подтверждения опасна. Нужен механизм: ИИ предлагает действие → пользователь подтверждает через <action> да/нет → действие выполняется или отменяется.

**Решение.** Серверный перехват опасных методов:
1. SAFE_METHODS (чтение, навигация) выполняются автоматически
2. Опасные методы (write_file, create, delete, set_property) перехватываются
3. В body.pendingAction сохраняются вызовы + контекст
4. Пользователь видит описание + <action> «Выполнить»
5. При подтверждении (confirm:true) — вызовы выполняются без обращения к модели
6. При отказе (confirm:false) — добавляется tool_result «отменено», модель продолжит диалог

**Архитектура.** Логика выполнения tool_call вынесена в executeToolCall() — единая функция для всех вызовов.

**Правило.** Опасные действия требуют подтверждения через <action>. Безопасные (чтение, навигация) — выполняются автоматически.

**На будущее.** Шкала доверия — уровни автоподтверждения (low/mid/high trust) вместо бинарного да/нет.

## ## Удаление режима Plan/Act — 2026-07-16

**Контекст.** Режим Plan/Act (кнопка run) был реализован в серверном цикле prompt: при actMode=false опасные методы (write_file, create, set_property, delete) блокировались с сообщением «нажмите кнопку run». Но кнопка run (oda-toggle) была удалена из UI при рефакторинге панели управления. Без кнопки блокировка бессмысленна и приводит к тупику: ИИ предлагает план → пользователь отвечает «Нет» → ИИ пытается выполнить действие → блокировка.

**Решение.** Полностью убрать режим Plan/Act:
1. Удалить блокировку isDangerousMethod/SAFE_METHODS из цикла prompt
2. Удалить переменную actMode из execute()
3. Удалить секцию «Режим Plan/Act» из SYSTEM_PROMPT
4. Клиентский send() не передаёт act (уже не передавал)

**Правило.** ИИ работает свободно, без серверной блокировки. Пользователь контролирует через <action> да/нет перед выполнением.

## ## SYSTEM_PROMPT: удаление дублирования инструментов — 2026-07-16

**Контекст.** SYSTEM_PROMPT содержал ~115 строк с описанием инструментов (get_schema, read_file, write_file, create, delete и др.), навигации и создания элементов системы. Но инструменты уже передаются через \`functions\` (function calling) — \`buildFunctionsFromSchema\` генерирует их автоматически из схемы методов класса. Это дублировало информацию и расходовало токены.

**Решение.** Удалить из SYSTEM_PROMPT все секции с детальным описанием инструментов, навигации и создания элементов. Заменить одной краткой секцией «Инструменты» (6 строк): вызови get_schema, не указывай path, navigate/reset_context.

**Правило.** Инструменты передаются через function calling. Не дублировать их текстовое описание в системном промпте.

## Тег <action>: только вопросы да/нет — 2026-07-16

**Контекст.** Тег <action> использовался для трёх типов: accept_plan (подтверждение плана), accept_result (принятие результата), ok (вопрос да/нет). Это перегружало интерфейс — план управляется собственным UI с шагами, а результат не требует отдельной кнопки.

**Решение.** Упростить <action> до одного сценария: однозначный вопрос да/нет. Убрать поля type, accept_plan, accept_result. Клиентский onAction() отправляет «Да», onCancelAction() — «Нет».

**Правило.** Кнопка <action> — только для бинарных вопросов. План — через UI плана. Завершение работы — просто текстовый ответ.

## # Журнал архитектурных решений

## Function calling: нативный вместо текстового парсинга — 2026-07-16

**Контекст.** ИИ-агент работал через текстовый парсинг `<tool_call>{"method":"...","args":{...}}</tool_call>` из ответа модели. Это ненадёжно: модели ломают JSON, путают кавычки, забывают теги. Платформа уже имела `streamChat` с поддержкой `functions` (OpenAI-compatible), но `prompt` метод не использовал эту возможность.

**Решение.** Внедрить нативный function calling:
1. `buildFunctionsFromSchema()` преобразует схему методов (из `get_schema`) в формат `[{name, description, parameters}]`
2. `prompt` передаёт `functions` + `function_call:'auto'` в `streamChat`
3. Цикл обрабатывает объекты `{type:'function_call', name, arguments}` из стрима
4. История диалога использует стандартный формат `role:'function'` с `name`
5. Fallback: текстовый парсинг `<tool_call>` сохранён для моделей без function calling

**Правило.** Схема методов класса — это и есть список инструментов агента. Не нужно дублировать описание инструментов в системном промпте — они передаются через `functions`.

## Баг: логи попадали в $work вместо meta_folder после ввода ролей — 2026-07-16

**Контекст.** После внедрения `get_storage({role})` файлы сохраняются в зону `$work` по роли пользователя. Но `_writeLogTo` в `file.js` передавал `role` в `storage.save_file()`, что направляло `data.logs` тоже в `$work`.

**Решение.** В `_writeLogTo` удаляем `role` из параметров и пишем от имени `globalThis.WORK`:
```js
const { role, ...rest } = log_param;
const systemLogParam = { ...rest, user: globalThis.WORK };
await storage.save_file(systemLogParam);
```

**Правило.** Логи — системная операция. Они всегда пишутся в `meta_folder/logs/` независимо от роли пользователя, сохранившего файл.

## Безопасность в объектной модели: роли admin/master/slaves — 2026-07-15

**Контекст.** Прежняя система (2 роли admin/users) размазана по `security.js` (731 строка), `class.js`, `folder.js`, `client/folder.js`. Логика наследования ролей была перепутана (masters не наследовался, slaves наследовался — оба наоборот).

**Модель (утверждена пользователем):**
- **admin** — системный администратор. Видит всё от точки вниз. Пишет SYSTEM (всё, кроме `$work`). Наследуется вниз.
- **master** — хозяин класса. Видит всё от точки вниз. Пишет MANAGEMENT (distributed `$work`). Наследуется вниз.
- **slave** — исполнитель. Видит только класс назначения. Пишет WORK (meta `$work`). НЕ наследуется.

**Решение.** Всю логику безопасности перенести в `$class` как методы и свойства. `security.js` — ликвидировать. Запись требует явного `params.role` от клиента (UI селектор ролей).

**Реализованные методы `$class`:**
- `roles(params)` — массив ролей через геттеры admins/masters/slaves
- `canSee(item, params)` — видимость элемента
- `canWrite(item, params)` — право записи (требует params.role)
- `allowAccess(params, level)` — единая проверка доступа
- `resolveZone(item)` — определение зоны: SYSTEM / MANAGEMENT / WORK

**Правило.** Безопасность — свойство класса, а не внешний модуль. Каждый класс самодостаточен.

## План-группа в микрочате: план прилипает к верху группы — 2026-07-14

**Контекст.** План рендерился отдельно над лентой (`<oda-chat-plan>` над thread), что сбивало с толку — план висел сам по себе, а не был связан с диалогом, который его породил.

**Решение.** Ввести понятие «план-группа»:
1. План парсится из ответа ИИ (`<plan>[...]</plan>`) → `msg.$plan` (`_loadTaskBody`)
2. `chatGroups` getter переорганизован: если ассистент создал план, текущая простая группа превращается в план-группу
3. План-группа имеет sticky header с промптом + планом (`oda-chat-plan` внутри группы)
4. Все последующие обмены (prompt + responses) добавляются в эту группу, пока план активен
5. При завершении всех шагов плана группа помечается `completed`, sticky снимается

**Структура `chatGroups`:**
```js
// План-группа
{
  type: 'plan',
  prompt: msg,
  plan: [...],
  completed: bool,
  exchanges: [
    { prompt: msg, responses: [...] },
    ...
  ]
}

// Простая группа
{
  type: 'simple',
  prompt: msg,
  responses: [...]
}
```

**Изменения в `$server/$folder/$file/$ai/handlers/preview/$handler/class.js`:**
1. CSS: `.plan-group-header` (sticky), `.plan-group-exchanges` (вложенность), `.plan-group-completed`
2. Шаблон: план-группа с `<template ~if="$for.item.type === 'plan'">`, простая группа — `<template ~if="$for.item.type !== 'plan'">`
3. `chatGroups` getter: логика создания план-группы и добавления обменов
4. `_loadTaskBody`: парсинг `<plan>` → `msg.$plan`

**Правило.** План всегда должен быть внутри группы, созданной промптом, который его вызвал. Не отделять план от контекста.